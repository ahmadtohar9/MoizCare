...
class Payroll extends CI_Controller {
    ...
    public function get_periods_json() {
        ...
        foreach($periods as $p) {
            ...
            $status_badge = '';
            if($p->status == 'draft') $status_badge = '<span class="px-2.5 py-1 rounded-full bg-gray-100 text-gray-700 text-xs font-bold uppercase tracking-wider">Draft</span>';
            else if($p->status == 'approved') $status_badge = '<span class="px-2.5 py-1 rounded-full bg-blue-100 text-blue-700 text-xs font-bold uppercase tracking-wider">Approved</span>';
            else if($p->status == 'paid') $status_badge = '<span class="px-2.5 py-1 rounded-full bg-green-100 text-green-700 text-xs font-bold uppercase tracking-wider">Paid</span>';
    ...
    public function export_sender($period_id)
    {
        ...
        // 2. Load Period Info
        $period = $this->db->get_where('payroll_periods', ['id' => $period_id])->row();
        if(!$period) show_error('Periode tidak ditemukan.', 404);
